...
This tool was built against one cohort and is separated from it in code and custody. A cohort
...
once here: a worked example in QUICKSTART.md carried a real cell type and a real cell count
from the study the tool was being built against, and the design examples carried its factors.
...
the directories where such strings land last and are looked for least. Two kinds of pattern:
  ...
  2. TERMS supplied from OUTSIDE the repository — the file named by $SCINTEGRATE_FORBIDDEN_TERMS — so
...
def terms() -> list:
    f = os.environ.get("SCINTEGRATE_FORBIDDEN_TERMS")
...
def main() -> int:
    ...
    print(f"scanned {sum(1 for _ in files())} files; {len(site)} site term(s) "
          f"{'from ' + os.environ['SCINTEGRATE_FORBIDDEN_TERMS'] if site else '(none supplied: set SCINTEGRATE_FORBIDDEN_TERMS to prove more)'}")
    for h in hits:
        print("  LEAK " + h)
    if hits:
